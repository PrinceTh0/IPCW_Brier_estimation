...
create.bs.dt <- function(data.list, tunedYN){
  ...
  dt.training.long[, "bs.mean" := mean(bs, na.rm = TRUE), by = times][, "bs.sd" := sd(bs, na.rm = TRUE), by = times]
  ...
  dt.test.long[, "bs.mean" := mean(bs, na.rm = TRUE), by = times][, "bs.sd" := sd(bs, na.rm = TRUE), by = times]
  ...
  dt.all.long[, "bs.mean" := mean(bs, na.rm = TRUE), by = times][, "bs.sd" := sd(bs, na.rm = TRUE), by = times]
...
saveRDS(seer_plot_data_list, file = ".../seer_semisynth_plot_data_list.rds")
