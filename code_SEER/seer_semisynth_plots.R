...
seer_res_data <- readRDS(file = paste0(path.data,"/seer_semisynth_plot_data_list.rds")) #path to "seer_plot_data_list.rds" data file
...
plot_cox <- ggplot(cox.dt.long.nt) +
  ...
  lims(x = c(0,149), y = c(0,0.3)) +
  labs(x = "Time [months]", y = "Brier Score", title = "Cox proportional hazards model") + # with title
  #labs(x = "Time", y = "Brier score") + # no title
  theme_classic() +
  theme.settings

ggsave(filename = "SEER_semisynth_cox.eps", plot = plot_cox, device = cairo_ps, path = path.plots, units = "in", width = 14, height = 8, dpi = 600)
...
ggsave(filename = "SEER_semisynth_xgboost.eps", plot = plot_xgboost, device = cairo_ps, path = path.plots, units = "in", width = 14, height = 8, dpi = 600)
...
ggsave(filename = "SEER_semisynth_random_forest.eps", plot = plot_random_forest, device = cairo_ps, path = path.plots, units = "in", width = 14, height = 8, dpi = 600)
